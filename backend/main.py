...
async def initiate_stk_push(request: STKPushRequest):
    ...
    try:
        ...
        if not (MPESA_CONSUMER_KEY and MPESA_CONSUMER_SECRET and MPESA_PASSKEY and MPESA_SHORTCODE):
            ...
            return STKPushResponse(success=False, message=reason)
        ...
        try:
            ...
            if stk_result.get("success") and checkout_request_id:
                ...
                reason = stk_result.get("message") or "M-Pesa did not return a CheckoutRequestID"
                logger.error(f"STK push not initiated: {reason}")
                supabase.table("payments").update({
                    "status": "failed",
                    "failure_reason": reason,
                    "updated_at": datetime.now().isoformat()
            ...
            reason = f"STK push failed: {str(e)}"
            logger.error(reason)
            supabase.table("payments").update({
                "status": "failed",
                "failure_reason": reason,
            ...
            return STKPushResponse(success=False, message="Payment initiation failed. Please try again.")
...
async def get_payment_status(checkout_request_id: str):
    ...
    try:
        ...
        if payment_status == "pending" and payment.get("checkout_request_id"):
            try:
    ...
    for payment in (pending.data or []):
        ...
        try:
            ...
            if result.get("success"):
                ...
                supabase.table("payments").update({
                    "status": "failed",
                    "failure_reason": result.get("reason") or "M-Pesa transaction failed",
                    "updated_at": datetime.now().isoformat()
                }).eq("id", payment["id"]).execute()
...
try:
    ...
    if _callback_path and _callback_path != "/api/public/payment/callback" and _callback_path != "/api/webhooks/mpesa":
        app.post(_callback_path, tags=["Public"])(payment_callback)
        logger.info(f"Registered configured M-Pesa callback path: {_callback_path}")
except Exception as e:
    logger.warning(f"Could not register custom M-Pesa callback path: {e}")
